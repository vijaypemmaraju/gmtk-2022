...
const ProjectileFiveStats = {
  speed: 5,
  damage: 5,
};

export default class ProjectileFive implements Projectile {
  ...
  instantiate(
    ...
    this.direction = direction.clone().scale(ProjectileFiveStats.speed);
  ...
  onHit(body: MatterJS.BodyType): void {
    this.explode();
    if (body.label === 'enemy') {
      ProjectileManager.applyEnemyDamage(ProjectileFiveStats.damage);
    }
  }
