...
const ProjectileThreeStats = {
  speed: 5,
  damage: 5,
};

export default class ProjectileThree implements Projectile {
  ...
  instantiate(
    ...
    this.direction = direction.clone().scale(ProjectileThreeStats.speed);
  ...
  onHit(body: MatterJS.BodyType): void {
    this.explode();
    if (body.label === 'enemy') {
      ProjectileManager.applyEnemyDamage(ProjectileThreeStats.damage);
    }
  }
