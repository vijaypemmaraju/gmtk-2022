...
const ProjectileFourStats = {
  speed: 5,
  damage: 5,
};

export default class ProjectileFour implements Projectile {
  ...
  instantiate(
    ...
    this.direction = direction.clone().scale(ProjectileFourStats.speed);
  ...
  onHit(body: MatterJS.BodyType): void {
    this.explode();
    if (body.label === 'enemy') {
      ProjectileManager.applyEnemyDamage(ProjectileFourStats.damage);
    }
  }
